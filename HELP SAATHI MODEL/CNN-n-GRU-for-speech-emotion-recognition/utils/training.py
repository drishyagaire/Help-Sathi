...
def transform_data_nepali(data):
    # Preprocessing is handled separately before training.
    return data
...
def transform_data(data):
    ...
    elif config.DATASET.lower() == "nepali":
        return transform_data_nepali(data)
    else:
...
def load_data_nepali(path):
    data = pd.read_csv(path)
    if 'source' in data.columns:
        data = data[data['source'].str.upper() == 'NEPALI'].reset_index(drop=True)
        del data['source']
