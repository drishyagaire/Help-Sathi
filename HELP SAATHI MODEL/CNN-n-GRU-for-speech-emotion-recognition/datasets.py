...
def wordclass_to_index(word, dataset=None):
    ...
    if dataset == "tess":
        ...
        return torch.tensor(config.NEPALI_EMOTION_TO_IDX[word])
    else:
...
def index_to_wordclass(index, dataset=None):
    ...
    if dataset == "tess":
        ...
        return config.NEPALI_IDX_TO_EMOTION[index]
    else:
        raise ValueError(f"Unknown dataset: {dataset}")
